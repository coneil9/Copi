    ...
    <div style={{ minHeight: '100vh', background: th.bg, color: th.ink }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 28px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: th.accent, margin: '0 0 4px' }}>Owner · Curriculum</p>
            <h1 style={{ fontFamily: '"Playpen Sans", "Fredoka", system-ui, sans-serif', fontSize: 38, fontWeight: 400, color: th.ink, margin: 0 }}>What your team is learning</h1>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => act.navigate && act.navigate('ai-review')}
              style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, fontWeight: 500, padding: '9px 18px', borderRadius: 999, background: th.bgInset, border: `1px solid ${th.line}`, color: th.ink, cursor: 'pointer' }}>
              ↑ Upload training docs
            </button>
            <button onClick={() => act.navigate && act.navigate('team')}
              style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, fontWeight: 500, padding: '9px 18px', borderRadius: 999, background: th.accent, border: 'none', color: th.onDark, cursor: 'pointer' }}>
              Assign to staff
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
          {[
            ...
            <div key={s.label} style={{ background: th.bgCard, borderRadius: th.card, padding: '18px 20px', border: `1px solid ${th.line}`, boxShadow: sh.card }}>
              ...
              <p style={{ fontFamily: '"Playpen Sans", "Fredoka", system-ui, sans-serif', fontStyle: 'italic', fontSize: 32, color: s.color, margin: '0 0 2px', lineHeight: 1 }}>{s.value}</p>
        ...
        {!setupDone && (
          <div style={{ background: `${th.accent}0e`, border: `1.5px solid ${th.accent}55`, borderRadius: th.card, padding: '18px 22px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <div>
              <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: th.accent, margin: '0 0 3px' }}>Setup required</p>
              <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 15, color: th.ink, margin: 0, fontWeight: 500 }}>
            ...
            <button onClick={() => act.navigate && act.navigate('ai-review')}
              style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, fontWeight: 600, padding: '9px 20px', borderRadius: 999, background: th.accent, border: 'none', color: th.onDark, cursor: 'pointer', flexShrink: 0 }}>
          ...
          <div>
            ...
            {onboardingModules.length > 0 && (
              <section style={{ marginBottom: 28 }}>
                <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: th.muted, marginBottom: 12 }}>Your cafe · Onboarding</p>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                            <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: th.accent, background: `${th.accent}15`, padding: '2px 8px', borderRadius: 999 }}>Onboarding</span>
                            <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, color: th.muted }}>Cafe-specific</span>
                          </div>
                          <p style={{ fontFamily: '"Playpen Sans", "Fredoka", system-ui, sans-serif', fontSize: 20, fontWeight: 400, color: th.ink, margin: '0 0 4px' }}>{mod.title}</p>
                          <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, color: th.muted, margin: 0 }}>
                            {(mod.lessonIds || []).length} milestones · {assignments.length} staff assigned · {done} completed
                          </p>
                        </div>
                        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                          <button onClick={() => act.navigate && act.navigate('ai-review')}
                            style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, fontWeight: 500, padding: '6px 14px', borderRadius: 999, background: th.bgInset, border: `1px solid ${th.line}`, color: th.muted, cursor: 'pointer' }}>
                            Edit
                          </button>
                        </div>
                      </div>
            ...
            <section>
              <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: th.muted, marginBottom: 12 }}>Copi standard curriculum</p>
              {volumes.map((v, i) => {
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
                          <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: th.muted, background: th.bgInset, padding: '2px 8px', borderRadius: 999 }}>{v.vol}</span>
                          {v.cert && <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, color: th.gold, background: `${th.gold}18`, padding: '2px 8px', borderRadius: 999 }}>→ {v.cert}</span>}
                        </div>
                        <p style={{ fontFamily: '"Playpen Sans", "Fredoka", system-ui, sans-serif', fontSize: 20, fontWeight: 400, color: th.ink, margin: '0 0 4px' }}>{v.name}</p>
                        <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, color: th.muted, margin: '0 0 10px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                        {v.assigned < teamSize && (
                          <button onClick={() => openAssign(v.id)}
                            style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 999, background: th.accent, border: 'none', color: th.onDark, cursor: 'pointer' }}>
                            Assign
                          </button>
                        )}
                        {v.assigned >= teamSize && v.assigned > 0 && (
                          <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, color: th.accent, background: `${th.accent}12`, padding: '5px 12px', borderRadius: 999 }}>✓ All assigned</span>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div>
            <div style={{ background: th.bgCard, borderRadius: th.card, border: `1px solid ${th.line}`, overflow: 'hidden', boxShadow: sh.card, marginBottom: 16 }}>
              <div style={{ padding: '14px 16px', borderBottom: `1px solid ${th.line}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: th.muted, margin: 0 }}>Lessons in motion</p>
              </div>
              {activity.length === 0 ? (
                <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, color: th.muted, padding: '20px 16px', textAlign: 'center', margin: 0 }}>No activity yet.</p>
              ) : activity.map((a, i) => (
                <div key={a.id || i} style={{ padding: '12px 16px', borderBottom: i < activity.length - 1 ? `1px solid ${th.line}` : 'none', display: 'flex', gap: 10 }}>
                  ...
                  <div>
                    <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, color: th.ink, margin: 0 }}><strong>{a.who}</strong> {a.action}</p>
                    <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 11, color: th.muted, margin: '1px 0 0', fontStyle: 'italic' }}>{a.label}</p>
                    <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, color: th.muted, margin: '2px 0 0', opacity: 0.65 }}>{relTime(a.ts)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: th.bgCard, borderRadius: th.card, border: `1px solid ${th.line}`, padding: '16px', boxShadow: sh.card }}>
              <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: th.muted, margin: '0 0 12px' }}>Quick actions</p>
              {[
                ...
                <button key={i} onClick={item.action}
                  ...
                  <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 16, color: th.accent, flexShrink: 0 }}>{item.icon}</span>
                  <div>
                    <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, fontWeight: 500, color: th.ink, margin: 0 }}>{item.label}</p>
                    <p style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 11, color: th.muted, margin: '1px 0 0' }}>{item.desc}</p>
                  </div>
                </button>
              ))}
            </div>
