...
export function AdminNav({ current, user, cafe }) {
  ...
  const Mono = ({ name: n, size = 34 }) => {
    ...
    return (
      <div style={{ width: size, height: size, borderRadius: '50%', background: th.accent, color: th.onDark, display: 'grid', placeItems: 'center', fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: size * 0.37, fontWeight: 700, flexShrink: 0 }}>
        {initials}
      </div>
    );
  };
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px', display: 'flex', alignItems: 'center', height: 58, gap: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginRight: 36, flexShrink: 0 }}>
          <span onClick={() => act.navigate && act.navigate('dashboard')}
            style={{ fontFamily: '"Playpen Sans", "Fredoka", system-ui, sans-serif', fontStyle: 'italic', fontSize: 24, color: th.accent, cursor: 'pointer', lineHeight: 1 }}>
            Copi.
          </span>
          <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 11, fontWeight: 500, color: th.muted, background: th.bgInset, padding: '2px 8px', borderRadius: 999 }}>
            {cafeName}
          </span>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13.5,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <span style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 13, color: th.muted }}>{displayName}</span>
          <button
            data-app-action="logout"
            style={{ fontFamily: '"Hanken Grotesk", "Inter", sans-serif', fontSize: 12, fontWeight: 500, padding: '5px 13px', borderRadius: 999, background: th.bgInset, border: `1px solid ${th.line}`, color: th.muted, cursor: 'pointer' }}>
            Log out
          </button>
