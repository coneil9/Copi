...
function AdminAnalyticsNew({ user = {} }) {
  ...
  const Mono = ({ name: n, size = 28 }) => {
    ...
    return <div style={{ width: size, height: size, borderRadius: '50%', background: th.accent, color: th.onDark, display: 'grid', placeItems: 'center', fontFamily: '"Inter", sans-serif', fontSize: size*0.37, fontWeight: 700, flexShrink: 0 }}>{initials}</div>;
  };
    ...
    <div style={{ minHeight: '100vh', background: th.bg, color: th.ink }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 28px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
          {/* Team progress table */}
          <div>
            <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: th.muted, marginBottom: 12 }}>Individual progress</p>
            <div style={{ borderRadius: th.card, border: `1px solid ${th.line}`, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 90px 110px 100px 90px 80px', background: th.bgInset, borderBottom: `1px solid ${th.line}`, padding: '0 4px' }}>
                {['Name','Progress','Certification','Status','Trend','Last active'].map(h => (
                  <div key={h} style={{ fontFamily: '"Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: th.muted, padding: '9px 10px' }}>{h}</div>
                ))}
              </div>

              {team.length === 0 ? (
                <div style={{ fontFamily: '"Inter", sans-serif', fontSize: 14, color: th.muted, padding: '24px', textAlign: 'center', background: th.bgCard }}>No staff enrolled yet.</div>
              ) : team.map((t, i) => {
                ...
                return (
                  <div key={t.email} style={{ display: 'grid', gridTemplateColumns: '2fr 90px 110px 100px 90px 80px', background: th.bgCard, borderBottom: i < team.length-1 ? `1px solid ${th.line}` : 'none', padding: '0 4px', alignItems: 'center' }}>
                    ...
                    <div style={{ padding: '12px 10px', display: 'flex', gap: 10, alignItems: 'center' }}>
                      ...
                      <div>
                        <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 13, fontWeight: 500, color: th.ink, margin: 0 }}>{t.name}</p>
                        <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 11, color: th.muted, margin: '1px 0 0' }}>{t.role}</p>
                      </div>
                    </div>
                    {/* Progress */}
                    <div style={{ padding: '12px 10px' }}>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 13, fontWeight: 600, color: th.ink, margin: '0 0 4px' }}>{t.pct}%</p>
                    ...
                    {/* Certification */}
                    <div style={{ padding: '12px 10px' }}>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 12, color: t.cert === '—' ? th.muted : th.accent }}>{t.cert === '—' ? 'Not yet' : t.cert}</p>
                    </div>
                    {/* Status */}
                    <div style={{ padding: '12px 10px' }}>
                      <span style={{ fontFamily: '"Inter", sans-serif', fontSize: 11, fontWeight: 600, background: fs.bg, color: fs.color, padding: '3px 9px', borderRadius: 999 }}>{fs.label}</span>
                    </div>
                    {/* Trend */}
                    <div style={{ padding: '12px 10px' }}>
                      <Trend dir={t.trend} />
                    ...
                    {/* Last active */}
                    <div style={{ padding: '12px 10px' }}>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 11, color: th.muted }}>{t.active}</p>
                    </div>
                  </div>
                );
              })}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                        <span style={{ fontFamily: '"Inter", sans-serif', fontSize: 10, fontWeight: 700, color: sevColor(g.sev), background: `${sevColor(g.sev)}18`, padding: '2px 7px', borderRadius: 999 }}>{g.sev}</span>
                        <span style={{ fontFamily: '"Inter", sans-serif', fontSize: 10, color: th.muted }}>{g.vol}</span>
                      </div>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 14, fontWeight: 500, color: th.ink, margin: '0 0 4px' }}>{g.topic}</p>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 12, color: th.muted, margin: 0 }}>{g.pct}% of team completed</p>
                    </div>
                    <button onClick={() => setExpandedGap(expanded ? null : i)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: th.accent, fontFamily: '"Inter", sans-serif', fontSize: 12, padding: '2px 6px', flexShrink: 0 }}>
                      {expanded ? 'Less' : 'Drill →'}
                    </button>
                  </div>
                  {expanded && (
                    <div style={{ marginTop: 12, padding: '12px 14px', background: `${th.accent}0a`, borderRadius: 10, border: `1px solid ${th.accent}22` }}>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: th.accent, margin: '0 0 6px' }}>Floor drill</p>
                      <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 13, color: th.ink, lineHeight: 1.55, margin: 0 }}>
            ...
            {cafe && (() => {
              ...
              return (
                <div style={{ marginTop: 16 }}>
                  <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: th.muted, marginBottom: 8 }}>Product updates</p>
                  {puMods.map(mod => {
                    ...
                    return (
                      <div key={mod.id} style={{ background: th.bgCard, borderRadius: 10, padding: '12px 14px', border: `1px solid ${urgentDeadline ? th.urgent||th.danger : th.line}`, marginBottom: 8 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                          <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 13, fontWeight: 500, color: th.ink, margin: 0 }}>{mod.title}</p>
                          <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 12, color: pct === 100 ? th.accent : th.muted, margin: 0 }}>{done}/{asgns.length}</p>
                        </div>
                        <div style={{ background: th.bgInset, borderRadius: 999, height: 5, overflow: 'hidden' }}>
